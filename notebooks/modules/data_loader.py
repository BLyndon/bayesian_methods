...
def get_vortex_data(path, asarray=False, batch_size=32):
    ...
    if asarray:
        ...
        train_ds = tf.data.Dataset.from_tensor_slices(
            x_train.astype('float32')).batch(batch_size)
        test_ds = tf.data.Dataset.from_tensor_slices(
            x_test.astype('float32')).batch(batch_size)
...
def get_mnist(img_shape=(28, 28, 1), batch_size=32, asarray=False, dataset='mnist'):
    ...
    x_train, y_train = preprocess(x_train, y_train, x_train.shape[0])
    x_test, y_test = preprocess(x_test, y_test, x_test.shape[0])

    if asarray:
        print('Return as Arrays')
        return (x_train, y_train), (x_test, y_test)
    else:
        train_ds = to_dataset(x_train, y_train, x_train.shape[0])
        test_ds = to_dataset(x_test, y_test, x_test.shape[0])
